...
namespace Microsoft.Azure.Devices.Edge.Agent.Core.Test.Planners
{
    ...
    public class HealthRestartPlannerTest
    {
        ...
        {
            (TestCommandFactory factory, Mock<IEntityStore<string, ModuleState>> store, IRestartPolicyManager restartManager, IServiceRegistry serviceRegistry, _) = CreatePlanner();

            Assert.Throws<ArgumentNullException>(() => new HealthRestartPlanner(null, store.Object, IntensiveCareTime, restartManager, serviceRegistry));
            Assert.Throws<ArgumentNullException>(() => new HealthRestartPlanner(factory, null, IntensiveCareTime, restartManager, serviceRegistry));
            Assert.Throws<ArgumentNullException>(() => new HealthRestartPlanner(factory, store.Object, IntensiveCareTime, null, serviceRegistry));
            Assert.Throws<ArgumentNullException>(() => new HealthRestartPlanner(factory, store.Object, IntensiveCareTime, restartManager, null));
            Assert.NotNull(new HealthRestartPlanner(factory, store.Object, IntensiveCareTime, restartManager, serviceRegistry));
        }
        ...
        static (TestCommandFactory factory, Mock<IEntityStore<string, ModuleState>> store, IRestartPolicyManager restartManager, IServiceRegistry serviceRegistry, HealthRestartPlanner planner) CreatePlanner()
        {
