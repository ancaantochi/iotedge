        ...
        public async Task SmokeTest()
        {
            ...
            try
            {
                using (var cts = new CancellationTokenSource(this.defaultTimeout))
                {
                    ...
                    var module = new DockerModule(Name, "1.0", ModuleStatus.Running, global::Microsoft.Azure.Devices.Edge.Agent.Core.RestartPolicy.OnUnhealthy, config, null, EnvVars, null);
        ...
        public async Task EdgeHubLaunch()
        {
            ...
            try
            {
                using (var cts = new CancellationTokenSource(this.defaultTimeout))
                {
                    ...
                    var module = new EdgeHubDockerModule("docker", ModuleStatus.Running, global::Microsoft.Azure.Devices.Edge.Agent.Core.RestartPolicy.Always, config, configurationInfo, EnvVars, null);
        ...
        public async Task UpstreamProtocolTest()
        {
            ...
            try
            {
                using (var cts = new CancellationTokenSource(this.defaultTimeout))
                {
                    ...
                    var loggingConfig = new DockerLoggingConfig("json-file", dockerLoggingOptions);
                    var config = new DockerConfig(Image, @"{""Env"": [""k1=v1"", ""k2=v2""], ""HostConfig"": {""PortBindings"": {""8080/tcp"": [{""HostPort"": ""80""}]}}}");
                    var module = new DockerModule(Name, "1.0", ModuleStatus.Running, global::Microsoft.Azure.Devices.Edge.Agent.Core.RestartPolicy.OnUnhealthy, config, null, EnvVars, services);
