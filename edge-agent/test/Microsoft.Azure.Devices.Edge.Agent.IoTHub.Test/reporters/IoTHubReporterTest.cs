    ...
    public class IoTHubReporterTest
    {
        ...
        public async void ReportShutdown()
        {
            using (var cts = new CancellationTokenSource(Timeout))
            {
                ...
                IEdgeAgentModule edgeAgent = new EdgeAgentDockerRuntimeModule(new DockerReportedConfig("image", string.Empty, "hash"), ModuleStatus.Running, 0, string.Empty, lastStartTimeUtc, DateTime.MinValue, ImagePullPolicy.OnCreate, new ConfigurationInfo("id"), new Dictionary<string, EnvVal>(), null);
                IEdgeHubModule edgeHub = new EdgeHubDockerRuntimeModule(ModuleStatus.Running, RestartPolicy.Always, new DockerReportedConfig("hubimage", string.Empty, "hash"), 0, string.Empty, DateTime.Now, DateTime.Now, 0, DateTime.Now, ModuleStatus.Running, ImagePullPolicy.OnCreate, new ConfigurationInfo("hub"), new Dictionary<string, EnvVal>(), null);
        ...
        public async void ReportShutdownWithOnlyEdgeAgentInState()
        {
            using (var cts = new CancellationTokenSource(Timeout))
            {
                ...
                IEdgeAgentModule edgeAgent = new EdgeAgentDockerRuntimeModule(new DockerReportedConfig("image", string.Empty, "hash"), ModuleStatus.Running, 0, string.Empty, lastStartTimeUtc, DateTime.MinValue, ImagePullPolicy.OnCreate, new ConfigurationInfo("id"), new Dictionary<string, EnvVal>(), new Dictionary<string, ServiceInfo>());
